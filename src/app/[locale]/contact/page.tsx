    ...
    <main className="min-h-screen bg-[#e5e5e5] text-[#141414]">
      ...
      <div className="mx-auto grid max-w-7xl gap-16 px-6 pb-24 pt-36 md:px-12 md:pt-44 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24 lg:px-16 xl:px-20">
        <section className="flex flex-col justify-between">
          <div>
            <p className="mb-7 font-mono text-xs uppercase tracking-[0.2em] text-[#e2551c]">{t("eyebrow")}</p>
            <h1 className="max-w-xl text-5xl font-medium leading-[0.98] tracking-[-0.04em] text-[#141414] sm:text-6xl lg:text-[5.2rem]">{t("title")}</h1>
            <p className="mt-8 max-w-md text-base leading-7 text-[#44474d]">{t("description")}</p>
          </div>

          <div className="mt-20 border-t border-neutral-900/10 pt-7 lg:mt-32">
            <p className="mb-5 font-mono text-[10px] uppercase tracking-[0.2em] text-[#44474d]">{t("directLine")}</p>
        ...
        <section className="border-t border-neutral-900/20 pt-7 lg:pt-9" aria-labelledby="form-heading">
          <div className="mb-10 flex items-baseline justify-between gap-6"><h2 id="form-heading" className="text-2xl font-medium tracking-tight text-[#141414] sm:text-3xl">{t("formTitle")}</h2><span className="font-mono text-xs text-[#44474d]">{t("required")}</span></div>
          <ContactForm />
        </section>
      </div>

      <footer className="flex flex-col gap-5 border-t border-neutral-900/10 px-6 py-7 text-xs text-[#44474d] sm:flex-row sm:items-center sm:justify-between md:px-12 lg:px-16 xl:px-20"><span>© {new Date().getFullYear()} Mpinger Groups</span><div className="flex gap-5"><Link className="transition-colors hover:text-[#e2551c]" href="/impressum">{nav("imprint")}</Link><Link className="transition-colors hover:text-[#e2551c]" href="/datenschutz">{nav("privacy")}</Link></div></footer>
    </main>
  );
}
